...
| <a id="t08"></a>**T08** | Credential theft or exfiltration by build code | Deterministic credential-path rules catch common explicit access. Build execution is a normal user inside a Bubblewrap boundary with a controlled view, private temporary paths, resource limits, and selective network policy. | 🟡 **Partially mitigated** | The allowlisted `cargo fetch --locked` shape receives bounded egress by default for its complete `makepkg` invocation. The package workspace and required build inputs remain visible; kernel escapes, explicit leases, toolchain compromise, or data encoded into outputs remain risks. |
...
The root administrator can explicitly enable `overrides.allow_unsafe`. This
