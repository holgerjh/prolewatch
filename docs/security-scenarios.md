...
| `srcinfo-checksum-mismatch` | Control | Ordinary stale generated metadata, modeled after a real AUR maintenance discrepancy | Approval-eligible HIGH `srcinfo-mismatch` plus a MEDIUM `vendor-provenance-weak`; blocks pending your decision | Compares only checksum arrays that are statically resolvable without executing the PKGBUILD; dynamic arrays remain unknown, and a conflicting `.SRCINFO` digest is not presented as the effective source binding. |
