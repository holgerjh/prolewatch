...
| `srcinfo-checksum-mismatch` | Control | Ordinary stale generated metadata, modeled after a real AUR maintenance discrepancy | Approval-eligible HIGH `srcinfo-mismatch`; blocks pending your decision | Compares only checksum arrays that are statically resolvable without executing the PKGBUILD; dynamic arrays remain unknown. |
