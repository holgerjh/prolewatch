...
func SemanticTextFindings(file, text string, executable, mandatory bool, bundle *ThreatBundle) []Finding {
	// Structured JSON/config checks run even for non-shell files. Shell candidates
	// then use a Bash AST so quoting, functions, pipelines, and substitutions are
	// interpreted structurally instead of by text signatures alone.
	...
	// First map calls to their enclosing makepkg phase. This context changes only
	// the explanatory network rule ID; recognized tools never receive a grant.
		...
		case *syntax.CallExpr:
			...
			case privilegeCommands[command]:
				add(semanticFinding(file, line, "critical", "privilege_escalation", "shell-privilege-command", evidence,
					"package-controlled code invokes a host privilege-escalation command", false))
			case hostPackageCommands[command]:
				add(semanticFinding(file, line, "critical", "privilege_escalation", "shell-host-package-manager", evidence,
					"package-controlled code invokes a host package manager or recursively invokes makepkg", false))
			}
			if command == "alias" {
			...
			if ecosystem, packages, ok := ecosystemInstall(command, args); ok {
				...
				// Recognising a package-installer call in attacker-authored shell
				// is recognition, not enforcement: the text can be obfuscated,
				// and the pattern has legitimate uses. It is described, at the
				// severity the location warrants, and it does not block.
				//
				// Recognised script text is described rather than blocked. Treating
				// it as structural proof creates a false-positive cliff and invites
				// an unsafe global bypass.
				severity, rationale := "high", "build phase invokes an ecosystem package installer and requires review"
				if installTime {
					severity, rationale = "critical", "installation script invokes an ecosystem package installer, which runs as root"
				}
				add(semanticFinding(file, line, severity, "build_hook", "shell-ecosystem-install", evidence, rationale, false))
			}
			...
			if (command == "source" || command == ".") && callContainsProcessDownloader(current) {
				add(semanticFinding(file, line, "critical", "remote_execution", "shell-remote-source", evidence,
					"remote content is sourced through command or process substitution", false))
			}
		case *syntax.BinaryCmd:
			...
			if downloadCommands[left] && interpreterCommands[right] {
				add(semanticFinding(file, int(current.Pos().Line()), "critical", "remote_execution", "shell-remote-pipeline",
					renderShellNode(current), "remote content is piped directly to an interpreter", false))
			}
...
func staticShellWord(word *syntax.Word) (string, bool) {
	// Return a value only for literal and quoted-literal parts. Expansions,
	// substitutions, arithmetic, and globs remain unknown rather than being
	// guessed into a security-sensitive command classification.
...
func unwrapCommand(words []string) (string, []string) {
	// Normalize common wrappers and static env assignments to the actual command
	// while refusing to interpret dynamic/empty words as meaningful argv.
	for len(words) > 0 {
...
// merely executable names. It is used only to explain the bounded
// build-network broker and select an isolated package-local cache; arbitrary
// curl/wget and generic shell commands must never inherit an automatic grant.
func knownBuildNetworkStep(command string, args []string) (string, bool) {
	...
	switch command {
	case "cargo":
		if len(args) == 2 && step == "fetch" && args[1] == "--locked" {
			return "cargo fetch --locked", true
		}
	case "go":
		if len(args) == 2 && step == "mod" && strings.ToLower(args[1]) == "download" {
...
func jsonManifestFindings(file, text string, bundle *ThreatBundle) []Finding {
	// Decode only security-relevant top-level fields. Lifecycle scripts, exact
	// dependency indicators, and registry overrides remain independent findings.
