...
func emitArchiveProgress(result *ArchiveScan, progress archiveProgressFunc) {
	if result != nil && progress != nil {
		progress(ArchiveProgress{Archives: result.Archives, Entries: result.Entries, UnpackedBytes: result.UnpackedBytes})
	}
}
...
func ArchiveFormat(head []byte) string {
	// Recognize standard on-disk magic: ZIP local header, gzip, bzip2, XZ, Zstd,
	// cpio ASCII/binary variants, and POSIX tar's "ustar" field at byte 257.
	// Names are only a hint; these bytes choose the parser.
	switch {
...
func scanArchiveWithPolicyDepth(reader io.ReadSeeker, displayName string, cfg Config, rules RuleEngine, depth, policyDepth int, progress archiveProgressFunc) ArchiveScan {
	// policyDepth is the configured semantic expansion depth for vendor content;
	// MaxArchiveDepth remains the hard safety ceiling. A negative policyDepth
	// means scan recursively until that safety ceiling.
	...
	// One 512-byte archive block covers every supported signature, including the
	// tar ustar marker at offsets 257..261.
	head := make([]byte, 512)
	...
	if result.Format == "cpio" {
		// makepkg/bsdtar can extract cpio, but Go's in-process scanner has no cpio
		// reader. Marking coverage incomplete prevents a false inspected claim.
...
// Decoder budgets. The scanner's entry, byte and depth limits count what comes
// *out* of a decoder; they say nothing about what it allocates on the way in,
// and a decompressor sizes its buffers from numbers the attacker wrote.
const (
	// xzDictionaryLimit caps the LZMA2 dictionary an xz member may declare.
	//
	// The declared size is allocated in full before a single byte is decoded,
	// and this library treats the declaration as a floor rather than a ceiling:
	// lzmafilter.go raises its configured DictCap to whatever the stream asks
	// for, up to the format's 4 GiB maximum. A few hundred well-formed bytes
	// therefore cost the scanner gigabytes. `xz -9` uses 64 MiB, so this is
	// twice the largest dictionary a real source archive carries.
	xzDictionaryLimit = 128 << 20
	// zstdWindowLimit caps the zstd decoder's window. Standard levels use at
	// most 8 MiB and `--ultra -22` uses 128 MiB; the library's own default
	// ceiling is far higher.
	zstdWindowLimit = 128 << 20
	// zstdMemoryLimit caps decoded output. The library defaults to 64 GiB.
	zstdMemoryLimit = 4 << 30
)
...
func compressedReader(source io.Reader, head []byte) (io.Reader, io.Closer, error) {
	...
	case bytes.HasPrefix(head, []byte{0xfd, '7', 'z', 'X', 'Z'}):
		if err := checkXZDictionary(head); err != nil {
			return nil, nil, err
		}
	...
	case bytes.HasPrefix(head, []byte{0x28, 0xb5, 0x2f, 0xfd}):
		r, err := zstd.NewReader(buffered,
			...
			zstd.WithDecoderMaxMemory(zstdMemoryLimit),
			zstd.WithDecoderConcurrency(1))
		if err != nil {
			return nil, nil, err
		}
		return r, r.IOReadCloser(), nil
	default:
		return buffered, nil, nil
	}
}

// checkXZDictionary reads the LZMA2 dictionary size an xz stream declares and
// refuses anything above the limit, before the decoder allocates it.
//
// Only the stream header and the first block header are parsed - the smallest
// amount of format knowledge that answers the question. A header that does not
// fit in the inspected prefix is refused rather than guessed at; the result is
// a coverage finding, not a silent pass.
func checkXZDictionary(head []byte) error {
	const streamHeaderSize = 12
	if len(head) <= streamHeaderSize {
		return errors.New("xz stream ends before its first block header")
	}
	block := head[streamHeaderSize:]
	// The first byte encodes the block header size in four-byte units. Zero
	// marks the stream index instead, which decodes nothing.
	if block[0] == 0 {
		return nil
	}
	size := (int(block[0]) + 1) * 4
	if size > len(block) {
		return errors.New("xz block header is not within the inspected prefix")
	}
	cursor := block[1:size]
	if len(cursor) == 0 {
		return errors.New("truncated xz block header")
	}
	flags := cursor[0]
	cursor = cursor[1:]
	var err error
	for _, present := range []bool{flags&0x40 != 0, flags&0x80 != 0} {
		...
		if _, cursor, err = xzVarint(cursor); err != nil {
			return err
		}
	}
	for range int(flags&0x03) + 1 {
		var id, length uint64
		if id, cursor, err = xzVarint(cursor); err != nil {
			return err
		}
		if length, cursor, err = xzVarint(cursor); err != nil {
			return err
		}
		if length > uint64(len(cursor)) {
			return errors.New("xz filter properties overrun the block header")
		}
		properties := cursor[:length]
		cursor = cursor[length:]
		// Filter 0x21 is LZMA2; its single property byte encodes the dictionary.
		if id != 0x21 || len(properties) != 1 {
			continue
		}
		encoded := properties[0]
		if encoded > 40 {
			return errors.New("xz member declares an invalid LZMA2 dictionary size")
		}
		dictionary := uint64(1) << 32
		if encoded < 40 {
			dictionary = uint64(2|(encoded&1)) << (encoded/2 + 11)
		}
		if dictionary > xzDictionaryLimit {
			return fmt.Errorf("xz member declares a %d MiB LZMA2 dictionary, above the %d MiB limit",
				dictionary>>20, uint64(xzDictionaryLimit)>>20)
		}
	}
	return nil
}

// xzVarint decodes one xz multibyte integer and returns the rest of the buffer.
func xzVarint(data []byte) (uint64, []byte, error) {
	var value uint64
	...
	return 0, nil, errors.New("malformed xz multibyte integer")
}
...
func scanZip(at io.ReaderAt, size int64, display string, cfg Config, rules RuleEngine, result *ArchiveScan, depth, policyDepth int, progress archiveProgressFunc) error {
	...
	for _, member := range zr.File {
		...
		emitArchiveProgress(result, progress)
		// General-purpose ZIP flag bit 0 means the member is encrypted; no bytes
		// are available for deterministic inspection without a password.
		if member.Flags&1 != 0 {
		...
		if mode&os.ModeSymlink != 0 {
			...
			// A 4096-byte target ceiling matches path field budgets; the extra byte
			// distinguishes exactly-at-limit content from an oversized target.
...
func scanArchiveMember(source io.Reader, declared int64, name, virtual string, mode uint32, cfg Config, rules RuleEngine, result *ArchiveScan, depth, policyDepth int, progress archiveProgressFunc) error {
	...
	// An 8 KiB prefix is enough for archive/executable magic and conservative
	// text classification while the combined reader still scans the whole member.
	...
	if LooksLikeArchive(virtual, first) {
		if policyDepth >= 0 && depth+1 >= policyDepth {
			// Deliberate vendor-depth omission is consumed but not reported as a
			// coverage failure; provenance records make that policy boundary visible.
		...
		nested := scanArchiveWithPolicyDepth(nestedFile, virtual, cfg, rules, depth+1, policyDepth, func(value ArchiveProgress) {
			if progress != nil {
				progress(ArchiveProgress{Archives: result.Archives + value.Archives, Entries: result.Entries + value.Entries,
					UnpackedBytes: result.UnpackedBytes + value.UnpackedBytes})
			}
		})
	...
	if probablyText(first) {
		...
		var sample []byte
		var total int64
		if structuredPacmanMetadata(name) {
			sample, total, err = readTextSample(io.TeeReader(combined, validator), cfg.Limits.MaxTextPerFile)
		} else {
		...
		text := safe.ValidUTF8OrReplacement(sample)
		if !structuredPacmanMetadata(name) {
...
func spoolNestedArchive(source io.Reader, declared int64) (*os.File, error) {
	// Nested parsers need seek/random access. Copy exactly the header-declared
	// bytes to a private temporary inode and reject short or overlong streams.
...
func scanCompressedStream(source io.Reader, head []byte, display string, cfg Config, rules RuleEngine, result *ArchiveScan, progress archiveProgressFunc) error {
	// A compression wrapper without a tar/zip container is treated as one virtual
	// member. The +1 counting limit makes overflow observable without unbounded
	// decompression.
	decompressed, closer, err := compressedReader(source, head)
	if err != nil {
		return err
	...
	var findings []Finding
	var sample []byte
	if structuredPacmanMetadata(display) {
		sample, _, err = readTextSample(io.TeeReader(combined, validator), cfg.Limits.MaxTextPerFile)
	} else {
...
// structuredPacmanMetadata identifies package-manager data whose words are not
// shell commands. Generic code signatures turn ordinary BUILDINFO fields such
// as buildenv=(!distcc color !ccache check !sign) and installed package names
// such as curl into execution findings. These members are still consumed in
// full, validated as text and checked by their format-specific parsers.
func structuredPacmanMetadata(name string) bool {
	switch path.Base(name) {
	case ".PKGINFO", ".BUILDINFO", ".MTREE":
		return true
	default:
		return false
	}
...
func readTextSample(reader io.Reader, sampleLimit int64) ([]byte, int64, error) {
	if sampleLimit < 0 {
		return nil, 0, errors.New("negative text sample limit")
	}
	...
	for {
		...
		if n > 0 {
			...
			if int64(len(sample)) < sampleLimit {
				take := int(sampleLimit - int64(len(sample)))
				if take > n {
					take = n
				}
...
// privilegedPackagePath consults the canonical registry so the artifact
// scanner and privileged-integration gate cannot diverge.
func privilegedPackagePath(name string) bool { return IsPrivilegedSurface(name) }
...
func artifactMetadataFindings(name, text, virtual string) []Finding {
	...
	if base == ".PKGINFO" {
		...
		// Parsed, not searched. `strings.Contains(line, "mode=4")` matched
		// mode=400 and mode=444 - a read-only file and a world-readable one -
		// and turned each into a critical hard block no approval can cross. The
		// target toolchain emits exactly those for ordinary package content.
		ParseMTree(text, func(index int, line string, record MTreeRecord) {
			if !MTreePrivileged(record) {
				return
			}
			ln := index + 1
...
func printableStrings(data []byte) string {
	// Four printable bytes is the conventional minimum that keeps bounded binary
	// evidence useful without flooding review input with isolated byte noise.
	var lines []string
	var current []byte
	flush := func() {
		if len(current) >= 4 {
...
func binaryFormat(data []byte) string {
	// ELF uses 0x7f + "ELF"; PE starts with the DOS "MZ" signature. The four
	// Mach-O constants cover 32/64-bit files in native and byte-swapped order.
