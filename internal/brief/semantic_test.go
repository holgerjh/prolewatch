...
// The 2018 incident pattern and an in-build sudo call: both reported at
// critical severity, neither blocking.
//
// What contains a curl-pipe-shell in package() is the sandbox - no network
// unless the user brokered it, no real home, no host filesystem - not the fact
// that a shell parser recognised the shape. A shape can be rewritten; the
// sandbox cannot be talked around. So the finding's job is to tell the user it
// is there, and the user can consent to it after reading the briefing.
func TestSemanticShellRecognitionIsCriticalButNotBlocking(t *testing.T) {
	...
	for _, rule := range []string{"shell-remote-pipeline", "shell-privilege-command"} {
		...
		if finding == nil {
			t.Fatalf("%s was not reported at all in %#v", rule, findings)
		}
		if finding.Severity != "critical" {
			t.Fatalf("%s severity = %q, want critical", rule, finding.Severity)
		}
		if finding.HardBlock {
			t.Fatalf("%s hard-blocked; recognised shell shapes are describable, not enforceable", rule)
		}
	}
}

// An installer call in a scriptlet is the most severe thing this rule reports -
// it runs as root - and it is still described rather than blocked. Recognising
// a pattern in attacker-authored shell can be obfuscated around; hard blocks
// are reserved for properties that cannot be rewritten away.
func TestSemanticInstallScriptPackageManagerIsDescribedNotBlocked(t *testing.T) {
	...
	if finding == nil {
		t.Fatalf("install-time package manager was not reported at all: %#v", findings)
	}
	if finding.Severity != "critical" {
		t.Fatalf("install-time severity = %q, want critical", finding.Severity)
	}
	if finding.HardBlock {
		t.Fatal("a recognised shell pattern hard-blocked")
	}
}
...
`
	...
	steps := KnownNetworkSteps(findings, nil, "prepare")
	if !containsString(steps, "cargo fetch --locked") {
		t.Fatalf("locked Cargo fetch was not recognized: %#v", findings)
	}
	if !containsString(steps, "go mod download") {
		t.Fatalf("Go module download was not recognized as prompt context: %#v", findings)
	}
	for _, forbidden := range []string{"curl", "git submodule update", "npm ci"} {
...
func TestSemanticRequiresExactLockedCargoFetchShape(t *testing.T) {
	findings := SemanticTextFindings("PKGBUILD", `prepare() { cargo fetch --locked --target x86_64-unknown-linux-gnu; }`+"\n", false, true, testThreatBundle(t))
	if steps := KnownNetworkSteps(findings, nil, "prepare"); len(steps) != 0 {
		t.Fatalf("extended Cargo command was treated as the exact locked adapter: %#v", steps)
	}
}
...
func TestSemanticScopesKnownNetworkStepToMakepkgProfile(t *testing.T) {
	...
	if got := KnownNetworkSteps(findings, nil, "prepare"); len(got) != 1 || got[0] != "cargo fetch --locked" {
		t.Fatalf("prepare network steps=%#v", got)
	}
	if got := KnownNetworkSteps(findings, nil, "build"); len(got) != 1 || got[0] != "cargo fetch --locked" {
...
func TestThreatIOCMatchesCommandsAndManifest(t *testing.T) {
	...
	// The highest-signal finding the scanner produces, and still not a block:
	// the bundle goes stale, and the identifier is attacker-chosen.
	if finding := findingByRule(commandFindings, "threat-aur-2026-js-digest"); finding == nil || finding.Severity != "critical" || finding.HardBlock {
	...
	if finding := findingByRule(manifestFindings, "threat-aur-2026-atomic-lockfile"); finding == nil || finding.Severity != "critical" || finding.HardBlock {
...
func TestEmbeddedThreatBundleIdentity(t *testing.T) {
	...
	if identity.BundleVersion == "" || !safe.ValidHexDigest(identity.SHA256) {
