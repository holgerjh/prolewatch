...
func validateStructuredOutputNode(node map[string]any, path string) error {
	// Structured Outputs requires closed objects with every property required.
	// Walk arrays, unions, and local definitions recursively to enforce that rule
	// below the root as well as at the top level.
